...
public class RedisService : IRedisService
{
    ...
    public async Task<bool> RemoveWorkerFromAllModelsAsync(string taskId)
    {
        ...
        try
        {
            // Get the Redis server endpoint
            var endpoints = _db.Multiplexer.GetEndPoints();
            var endpoint = endpoints.FirstOrDefault();
            
            if (endpoint == null)
            {
                _logger.LogWarning("No Redis endpoint found");
                return false;
            }

            var server = _db.Multiplexer.GetServer(endpoint);
            
            // Scan all keys matching worker pattern: ocr:model:*:workers
    ...
    public async Task PublishEventAsync(string streamKey, OcrEvent ocrEvent)
    {
        try
        {
            string jsonData = JsonSerializer.Serialize(ocrEvent, _jsonOptions);
    ...
    public async Task ClearAllStreamsAsync()
    {
        try
        {
            // Xóa stream chung ocr:events
            await _db.KeyDeleteAsync("ocr:events:stream");
            _logger.LogInformation("Deleted stream key: ocr:events:stream");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error clearing stream key");
        }
    }
}
